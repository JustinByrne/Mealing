...
@section('content')
    ...
    <h2 class="font-sans break-normal text-gray-900 pt-6 pb-4 text-lg font-bold">Multifactor Authentication</h2>
    ...
    <div x-data="{ open: false }">
        @if ($user->get2faEnabled())
            <a href="#" @click="open = true" x-on:click.prevent class="hover:text-orange-400 hover:underline">Show QR Code</a>
            <div class="absolute top-0 left-0 flex items-center justify-center w-full h-full" style="background-color: rgba(0,0,0,.5);" x-show="open">
            <div class="h-auto p-4 mx-2 text-left bg-white rounded shadow-xl md:max-w-xl md:p-6 lg:p-8 md:mx-0" @click.away="open = false">
                <div class="pb-3">
                    {!! $user->twoFactorQrCodeSvg() !!}
                </div>
                <x-inputs.button @click="open = false">Close</x-inputs.button>
            </div>
        @endif
    </div>
