<header class="flex items-center bg-gray-800 px-8 flex-wrap" x-data="{ nav: false }">
    ...
    <div class="w-full md:inline-flex md:flex-grow md:w-auto" :class="{ 'hidden': !nav }">
        <nav class="flex flex-col md:inline-flex md:flex-row md:ml-auto">
            @auth
                <x-links.nav href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard*')">
                    Dashboard
                </x-links.nav>
                ...
                <div class="relative px-2 hidden md:block" x-data="{ userShow: false }" @click.away="userShow = false" @keydown.escape.window="userShow = false">
                    ...
                    <div class="right-0 bg-gray-100 border-2 border-blueGray-700 rounded py-2 text-gray-600 mt-3 shadow-xl lg:w-56 lg:absolute z-10" x-show.transition.opacity.duration.300ms="userShow">
                        <x-links.user-nav href="{{ route('profile') }}">
                            Profile
                        </x-links.user-nav>
                        <x-links.user-nav href="{{ route('profile.settings.account') }}">
                            Settings
                        </x-links.user-nav>
                        <x-links.user-nav href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                            Log Out
                        </x-links.user-nav>
                ...
                <div class="w-full md:hidden">
                    ...
                    <div class="flex flex-col">
                        <x-links.nav href="{{ route('profile') }}">
                            Profile
                        </x-links.nav>
                        <x-links.nav href="{{ route('profile.settings.account') }}">
                            Settings
                        </x-links.nav>
            ...
            @else
                <x-links.nav href="{{ route('login') }}">
                    Login
                </x-links.nav>
                <x-links.nav href="{{ route('register') }}">
                    Register
                </x-links.nav>
            @endauth
        </nav>
